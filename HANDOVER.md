...
- **版本：** v1.6.0（master，`7724023`，2026-04-12）
- **分支：** master
...
- **Benchmark：** opus deep 22/24, sonnet shallow 21/24（真問題 8/8 兩者皆正確，差異在 FP）
...
| — | Benchmark: opus deep vs sonnet shallow（24 cases, real model）| `docs/benchmark-v1.6.0.md` | 0 |
| — | `run_benchmark()` 加 `prompt_depth` 參數 | `evals/eval_runner.py` | 0 |
| — | tag `v1.6.0` + push + CI ✓ | — | 0 |
| — | 部署到 `~/.claude/scripts/` | — | 0 |
| — | GitHub About 更新（382 tests, context retrieval）| — | 0 |
...
- Lint (ruff): clean
- Benchmark: opus deep 22/24, sonnet shallow 21/24, true positive 8/8 both
- 六訊號對齊 + 部署完成
