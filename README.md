...
- `protection` — optional block wrapper with `user_message` talking points, `agent_task`, `risk_summary`, `rerun_protocol`, and low-weight intent metadata. The `user_message` value is source material for an agent to summarize in its own words, not text to paste verbatim. It is added after the review decision and does not change schema version.
